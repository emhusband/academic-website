(() => {
  ...
  const T=(x,y,t,cls='r28-label')=>{const n=E('text',{x,y,class:cls});n.textContent=t;return n};
  const L=(x1,y1,x2,y2,cls='r28-line')=>E('line',{x1,y1,x2,y2,class:cls});
  const C=(cx,cy,r,cls='r28-node')=>E('circle',{cx,cy,r,class:cls});
  const S=()=>E('svg',{viewBox:'0 0 520 360',class:'r28-svg',role:'presentation'});
  ...
  const fadeScene=(scene,on)=>scene.classList.toggle('r28-scene-on',on);

  function grammar(box){
    const s=S(), scene=G('r28-scene'); s.append(scene); box.append(s);
    // Two identical rigid Vs. The lower constituent's apex is the right terminal of the upper V.
    const lower=G('r28-structure r28-lower');
    lower.append(L(330,176,285,216,'r28-branch'),L(330,176,375,216,'r28-branch'));
    const upper=G('r28-structure r28-upper');
    upper.append(L(285,116,240,156,'r28-branch'),L(285,116,330,156,'r28-branch'));
    scene.append(lower,upper);

    // Relevant semantic material starts visibly displaced from its terminals.
    const P=T(285,223,'P','r28-symbol r28-relevant r28-P');
    const x=T(375,223,'x','r28-symbol r28-relevant r28-x');
    const ex=T(240,163,'∃','r28-symbol r28-relevant r28-ex');
    // Distractors make the field genuinely underdetermined before structure asserts itself.
    const distractors=[
      T(118,102,'∀','r28-symbol r28-distractor r28-float-a'),
      T(430,88,'e','r28-symbol r28-distractor r28-float-b'),
      T(104,260,'s','r28-symbol r28-distractor r28-float-c'),
      T(438,265,'Q','r28-symbol r28-distractor r28-float-d'),
      T(340,70,'Op','r28-symbol r28-distractor r28-float-e')
    ];
    ...
    if(reduced){fadeScene(scene,true);allSymbols.forEach(n=>n.classList.add('on'));distractors.forEach(n=>n.classList.add('dim'));lower.classList.add('on');upper.classList.add('on');P.classList.add('placed');x.classList.add('placed');ex.classList.add('placed')} else box._r28Start=cycle;
  }

  function expectation(box){
    const s=S(),scene=G('r28-scene');s.append(scene);box.append(s);
    scene.append(L(48,250,472,250,'r28-axis'));
    const words=[T(76,242,'w₁','r28-word on'),T(200,242,'w₂','r28-word'),T(324,242,'w₃','r28-word'),T(448,242,'w₄','r28-word')];scene.append(...words);
    const paths=[
    ...
    const ds=paths.map((d,i)=>{const g=G(`r28-dist r28-dist-${i}`);g.append(E('path',{d,class:'r28-dist-fill'}),E('path',{d:d.replace(/ Z$/,''),class:'r28-dist-edge'}));scene.append(g);return g});
    ...
    if(reduced){fadeScene(scene,true);words.forEach(w=>w.classList.add('on'));ds[2].classList.add('on')} else box._r28Start=cycle;
  }

  function alternatives(box){
    const s=S(),scene=G('r28-scene');s.append(scene);box.append(s);
    const w1=T(205,188,'w₁','r28-word on'),w2=T(405,188,'w₂','r28-word');scene.append(w1,w2);
    const pts=[[112,84,17],[205,66,12],[298,102,22],[105,188,10],[292,188,14],[132,282,20],[225,298,11],[310,266,17]];
    const alts=pts.map(([x,y,r],i)=>{const g=G(`r28-alt r28-alt-${i}`);g.append(L(205,183,x,y,'r28-alt-line'),C(x,y,r,i%3===0?'r28-node r28-teal-node':'r28-node'));scene.insertBefore(g,w1);return g});
    const links=[[298,102],[292,188],[310,266]].map(([x,y])=>{const p=E('path',{d:`M405 183 C360 170 ${x+20} ${y-8} ${x} ${y}`,class:'r28-coherence'});scene.append(p);return p});
    ...
    if(reduced){fadeScene(scene,true);alts.forEach(a=>a.classList.add('on'));w2.classList.add('on');links.forEach(l=>l.classList.add('on'))} else box._r28Start=cycle;
  }

  function memory(box){
    const s=S(),scene=G('r28-scene');s.append(scene);box.append(s);
    function bundle(cx,cy,labels,cls){
      const g=G(`r28-bundle ${cls}`);
      const pts=labels.length===3
        ? [[cx,cy-38],[cx-38,cy+28],[cx+38,cy+28]]
        : [[cx-34,cy-30],[cx+34,cy-30],[cx-34,cy+30],[cx+34,cy+30]];
      if(labels.length===3){
        g.append(L(pts[0][0],pts[0][1],pts[1][0],pts[1][1],'r28-memory-line'),L(pts[0][0],pts[0][1],pts[2][0],pts[2][1],'r28-memory-line'),L(pts[1][0],pts[1][1],pts[2][0],pts[2][1],'r28-memory-line'));
      } else {
        g.append(L(pts[0][0],pts[0][1],pts[1][0],pts[1][1],'r28-memory-line'),L(pts[0][0],pts[0][1],pts[2][0],pts[2][1],'r28-memory-line'),L(pts[1][0],pts[1][1],pts[3][0],pts[3][1],'r28-memory-line'),L(pts[2][0],pts[2][1],pts[3][0],pts[3][1],'r28-memory-line'));
      }
      pts.forEach(([x,y],i)=>{g.append(C(x,y,15,`r28-memory-node node-${i}`),T(x,y+5,labels[i],`r28-feature feature-${i}`))});
      return g;
    }
    const old=bundle(145,185,['f₁','f₂','f₃'],'r28-old');
    const cur=bundle(390,185,['f₄','f₅','f₆','f_'],'r28-current');
    scene.append(old,cur);
    // Current bottom-right feature retrieves old top-right f2.
    const retrieve=E('path',{d:'M424 215 C355 285 270 275 183 213',class:'r28-retrieve'});scene.append(retrieve);
    const sourceNode=old.querySelector('.node-1'),sourceLabel=old.querySelector('.feature-1'),target=cur.querySelector('.feature-3');
    const reset=()=>{fadeScene(scene,false);old.classList.remove('seen','decayed');cur.classList.remove('seen');retrieve.classList.remove('on');sourceNode.classList.remove('react');sourceLabel.classList.remove('react');old.classList.remove('retrieving');target.classList.remove('resolved');target.textContent='f_'};
    const cycle=()=>{
      clear(box);reset();
      later(box,()=>fadeScene(scene,true),400);
    ...
    if(reduced){fadeScene(scene,true);old.classList.add('seen','decayed');cur.classList.add('seen');retrieve.classList.add('on');sourceNode.classList.add('react');sourceLabel.classList.add('react');target.textContent='f₂';target.classList.add('resolved')} else box._r28Start=cycle;
  }
  ...
  if(!reduced){
    const io=new IntersectionObserver(entries=>entries.forEach(entry=>{
      ...
      if(entry.isIntersecting){
        clear(box);
        requestAnimationFrame(()=>requestAnimationFrame(()=>box._r28Start?.()));
      } else {
        clear(box);
      }
    ...
    window.addEventListener('pageshow',()=>boxes.forEach(box=>{
      ...
      if(r.bottom>0 && r.top<innerHeight) { clear(box); requestAnimationFrame(()=>box._r28Start?.()); }
    }));
  }
})();
